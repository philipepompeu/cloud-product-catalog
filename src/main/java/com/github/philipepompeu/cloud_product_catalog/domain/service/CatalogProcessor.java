...
public class CatalogProcessor {
    ...
    public void process(String ownerId){
        ...
        for (CategoryDto dto : (Iterable<CategoryDto>) categories.stream()::iterator) {            
            dto.setProducts(productRepository.findByCategoryId(dto.getId()).orElse(List.of()).stream().map(entity-> ProductEntity.toDTO(entity)).toList());
        }

        if (categories.size() > 0) {
            CatalogDto catalog = new CatalogDto(categories);
            try {
